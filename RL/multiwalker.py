''' 
bipedal walker training using reinforcement learning

source: https://shiva-verma.medium.com/teach-your-ai-how-to-walk-5ad55fce8bca
'''
...
'''
actor network
- 2 hidden layers (size 600 and 300 respectively)
- batch norm and ReLU activation
- tanh activation on the final layer cuz in range (-1, 1)
'''
class Actor(nn.Module):
    ...
    def reset_parameters(self):
        # this slightly improved the performance compared to default
...
'''
critic network
- 2 hidden layers for state
- 1 hidden layer for action
- batch norm and ReLU activation
'''
...
class Agent():
    ...
    def learn(self, experiences, gamma):
        ...
        Q_targets = r + mu * critic_target(next_state, actor_target(next_state))
        where:
...
env = multiwalker_v8.env(n_walkers=3, position_noise=1e-3, angle_noise=1e-3, forward_reward=1.0, terminate_reward=-100.0, fall_reward=-10.0, shared_reward=True,
...
def ddpg(episodes, step, pretrained, noise):
    ...
    '''torch.save(agent.actor_local.state_dict(), '0checkpoint_actor.pth')
    torch.save(agent.critic_local.state_dict(), '0checkpoint_critic.pth')
    torch.save(agent.actor_target.state_dict(), '0checkpoint_actor_t.pth')
    torch.save(agent.critic_target.state_dict(), '0checkpoint_critic_t.pth')'''

    save_frames_as_gif(frames, filename='RL_pretrained.gif')
    print('Training saved')
    return reward_list


scores = ddpg(episodes=100, step=2000, pretrained=1, noise=1)
