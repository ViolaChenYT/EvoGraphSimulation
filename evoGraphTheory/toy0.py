...
def inrange(a,b,r):
  '''check if 2 centers of circle (a,b) are in range r, inrage -> True
  @params a, b: float * float, pair of coordinates;
...
class Agent():
  '''each agent moves according to a brownian motion in the map
  '''
  # initialization
  ...
  def move(self):
    ...
    # in polling state, it will move in the general direction of the potential target, with variation of a standard normal in radian
      ...
      if self.speculation[0] < 0 or self.speculation[1] < 0:
        print("YEEEEEEEEEET")
...
print(x)
print(frac_committed)
plt.plot(x, frac_free, label = 'no idea')
plt.plot(x, frac_polling, label = 'polling')
plt.plot(x, frac_committed, label = 'committed')
plt.legend()
# Display a figure.
plt.show()
