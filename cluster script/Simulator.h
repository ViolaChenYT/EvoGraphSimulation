...
void Simulator::simulate(double s = 0)
{
    ...
    int *ntoi = new int[popsize]; // what does this do :///
    int *iton = new int[popsize]; // index to node
    ...
    permute(iton, ntoi, populations[0], index2); // swaps the mutant to last index
    int t = 0;
    ...
    while (populations[0] != 0 && populations[1] != 0)
    {
        ...
        {
            --populations[1];
            index2 = populations[0]; // the no. of WT left
